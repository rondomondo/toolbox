...
SRC=/usr/local/bin/cast2gif

# If stdout is a pipe, stream the binary directly — the user is doing:
#   docker run ... install > cast2gif && chmod +x cast2gif
if [[ ! -t 1 ]]; then
    cat "$SRC"
    exit 0
fi
...
printf "\n"
printf "${BOLD}${CYAN}Install cast2gif${RESET}\n"
printf "\n"
printf "Run the following command in the directory where you want to install:\n"
printf "\n"
printf "  ${BOLD}docker run --rm ghcr.io/rondomondo/cast2gif install > cast2gif && chmod +x cast2gif${RESET}\n"
printf "\n"
printf "Then to install system-wide:\n"
printf "\n"
printf "      ${BOLD}install -m 755 cast2gif /usr/local/bin/cast2gif${RESET}\n"
printf "\n"
printf "Or use ${BOLD}./cast2gif${RESET} directly from the current directory.\n"
printf "\n"
